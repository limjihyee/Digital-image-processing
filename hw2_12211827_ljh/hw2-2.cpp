...
// img2.jpg가 위로 갈수록 어두워 지는 프로그램 작성하기 (산술 연산과 for 문으로 구현)
...
int dark_up(Mat img, int y, int x); //prototype
...
int main() {
	...
	for (int y = 0; y < src_img.rows; y++) {
		for (int x = 0; x < src_img.cols; x++) {
			...
			src_img.at<uchar>(y, x) = dark_up(src_img, y, x);
		}
	}
...
int dark_up(Mat img, int y, int x) {
	int e = img.at<uchar>(y, x) - (img.rows - y) / 2;
